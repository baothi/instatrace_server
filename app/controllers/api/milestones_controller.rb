class Api::MilestonesController < Api::ApiController
  ...
  def update        
    ...
    if params[:document].present?
      encoded_img = params[:document].sub('data:image/jpeg;base64,', '')
      io = FilelessIO.new(Base64.decode64(encoded_img))
      io.original_filename = "document_#{@milestone.shipment.hawb}.jpg"
      @milestone.milestone_documents.create(:name => io, :doc_type => params[:type])
    end
