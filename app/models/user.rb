class User < ActiveRecord::Base
  ...
  validates :password, :password_confirmation, :presence => true, :length => {:maximum => 255}, :on => :create, :unless => :sa?

  validates :login, :first_name, :last_name, :email, :phone,
    :presence => true, :length => {:maximum => 255}, :unless => :sa?
  ...
  has_many :locations, :foreign_key => :driver_id
  has_many :shipments, :through => :milestones
